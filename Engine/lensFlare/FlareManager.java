...
public class FlareManager {
    ...
    public void render(ICamera camera, Vector3f sunWorldPos){
        Vector2f sunCoords = convertToScreenSpace(sunWorldPos,camera.getViewMatrix(),camera.getProjectionViewMatrix());
        if (sunCoords == null){
            return;
        }
        ...
        if (brightness > 0){
            calcFlarePostions(sunToCenter,sunCoords);
            renderer.render(sunCoords,flareTextures,brightness);
        }
    }

    private void calcFlarePostions(Vector2f sunToCenter, Vector2f sunCoords){
